...
void main(void){
	...
	while(1){
		...
		while((temp_output == output || temp_output == ' ' ) && i < 2){
			temp_output = keypad_to_ascii();
		}
		//lcd_clear();
		output = temp_output;
		...
		{
			i = 0;
			if((first_digit == '7') && (second_digit == '1')){
				PORTB = 0b00111111;
				_delay_ms(5000);
				PORTB = 0b00000000;
			}
...
//     // 2. Ανίχνευση πατήματος (Αγνόησε το 0/κενό)
//     if (key != 0) {
        
//         // Καταγραφή ψηφίου
//         if (i == 0) {
//             first_digit = key;
//             lcd_clear();         // Καθαρισμός για νέα εισαγωγή
//             lcd_data(first_digit);
//             i++;
//         }
//         else if (i == 1) {
...
//         // 3. Σημαντικό: Περίμενε να αφεθεί το πλήκτρο (Single Press Logic)
//         // Αυτό καλύπτει την απαίτηση "πατήθηκε μόνο μια φορά" 

//         while (keypad_to_ascii() != ' '); 
//         _delay_ms(20); // Debounce release
//     }

//     // 4. Έλεγχος Κωδικού (μόλις έχουμε 2 ψηφία)
//     if (i == 2) {
//         _delay_ms(500); // Μικρή παύση για να προλάβει να δει το 2ο ψηφίο

//         // Περίπτωση Επιτυχίας (Π.χ. Ομάδα 71)
//         if ((first_digit == '7') && (second_digit == '1')) {
//             PORTB = 0b00111111; // Άναψε PB0-PB5
//             _delay_ms(3000);    // Απαίτηση: 3 sec 
//             PORTB = 0b00000000; // Σβήσε
//         } 
//         // Περίπτωση Αποτυχίας
//         else {
//             // Απαίτηση: Αναβόσβημα για 6 sec (500ms on/off) [cite: 1191]
//             // 6 sec / 1 sec (κύκλος) = 6 επαναλήψεις
//             for (uint8_t k = 0; k < 6; k++) {
//                 PORTB = 0b00111111;
//                 _delay_ms(500);
//                 PORTB = 0b00000000;
